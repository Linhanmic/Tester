...
import { parseHex, parseHexDefault, parseDataBytes as parseDataBytesUtil } from './utils/hexParser';
...
export class TesterParser {
  ...
  private parseConfigurationBlock(): ConfigurationBlock {
    ...
    while (this.currentLine < this.lines.length) {
      ...
      // 解析诊断配置（支持省略0x前缀，默认视为十六进制）
        ...
        const value = parseHexDefault(line.substring("tdiagnose_rid".length).trim());
        if (value !== null) {
          diagnose.requestId = value;
        }
      } else if (line.startsWith("tdiagnose_sid")) {
        const value = parseHexDefault(line.substring("tdiagnose_sid".length).trim());
        if (value !== null) {
          diagnose.responseId = value;
        }
      } else if (line.startsWith("tdiagnose_keyk")) {
        const value = parseHexDefault(line.substring("tdiagnose_keyk".length).trim());
        if (value !== null) {
  ...
  private parseTcansCommand(line: string): TcansCommand | null {
    ...
    if (parts.length === 4) {
      ...
      messageId = parseHexDefault(parts[0])!;
      dataStr = parts[1];
      ...
      if (isSecondDataBytes) {
        ...
        messageId = parseHexDefault(parts[1])!;
        dataStr = parts[2];
    ...
    const data = parseDataBytesUtil(dataStr);
  ...
  private parseExpectedValues(str: string): number[] {
    const parts = str.split("+");
    return parts.map((p) => parseHex(p.trim()) ?? 0);
  }
