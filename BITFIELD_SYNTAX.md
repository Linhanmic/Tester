...
位域函数定义将多个参数映射到CAN报文的不同位域。**支持一个函数同时发送多个不同CAN ID的报文**。
...
// 多CAN报文（使用分号分隔）
tbitfield 函数名 参数1="显示名1", 参数2="显示名2", ...: CAN_ID1, 位域1="参数1"/缩放因子; CAN_ID2, 位域2="参数2"
```
...
- **分号(;)**：用于分隔不同的CAN报文映射，实现一个函数调用发送多个CAN报文
...
**多CAN报文调用示例：**
```
// 一次调用同时发送车速和档位两个CAN报文
// 生成报文 0x144: 车速100km/h
// 生成报文 0x260: D档
...
6. **多CAN报文发送**：
   - 使用分号(;)分隔不同的CAN报文映射
   - 一个函数调用会按顺序发送所有定义的CAN报文
   - 所有报文使用相同的参数值，但映射到不同的CAN ID和位域
   - 如果任何一个报文发送失败，整个函数调用会立即返回失败
...
5. **复用性**：一次定义，多次调用
6. **批量发送**：支持多CAN报文，一次调用同时发送多个相关报文，简化测试脚本
...
ttitle=测试
  ...
  1 tstart=测试用例
    // 车辆状态 车速=100, 车速单位=km/h, 档位=D档
    tcans 0x144, 10 27 00 00 00 00 00 00, 0, 1
    tcans 0x260, 03 00 00 00 00 00 00 00, 0, 1
  tend
...
6. **多报文展开**：支持多CAN报文的位域函数，自动展开为多条tcans命令
